...
double easyMastery = 0.8;
double mediumMastery = 0.85;
double hardMastery = 0.9;
...
void drawErrors(int difficulty, int currentEmail) {
  fill(50, 130);
  rect(0, 0, 1280, 720);
  if (difficulty == 1) {
...
void drawEmailText() {
  color blue = color(170, 203, 255, 0);
  fill(blue);
  rect(88, 86, 92, 87);
  rect(88, 191, 92, 87);
  rect(88, 297, 92, 87);
...
void drawDesktopText() {
  ...
  for (int i = 0; i < 10; i++) {
    ...
    color grayedout = color(185, 185, 185, 100);
    fill (grayedout);
    if (clicked[i] == 1) {
      rect(348, 95 + ydiff*i, 671, 28);
    }
...
void drawSenderAddress()
{
  color yellow = color(255, 239, 118);
  fill(yellow);
...
double BKT(double plStart, double pTransit,double PGuess, double PSlip, int [] responses)
{
    return knowledgeTrace(plStart, pTransit, PGuess, PSlip, responses);
}

double knowledgeTrace(double pL, double pT, double pG, double pS, int [] responses)
{
    double tracedPL = pL;
...
double correctTrace(double givenPL,double  pT, double pG, double pS){
...
double wrongTrace(double givenPL, double pT, double pG, double pS){
